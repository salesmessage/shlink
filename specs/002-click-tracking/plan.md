...
| Fork discipline | This is a maintained fork of `shlinkio/shlink`. Keep every change under `module/`, `config/`, `data/` or `bin/` additive and narrow so upstream merges stay cheap, and call out a permanent divergence in the PR description with the JIRA key. Note the **known-red baseline** (58 phpcs errors, 18 phpstan errors, 4 failures and 7 errors in the unit suite on a clean checkout): establish it before blaming your change, and judge your work on the files you touched |
